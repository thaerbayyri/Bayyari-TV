...
public final class MovieDao_Impl extends MovieDao {
  ...
  @Override
  public Object replaceAllForServer(final int serverId, final List<MovieEntity> entities,
      final Continuation<? super Unit> $completion) {
    return DBUtil.performInTransactionSuspending(__db, (_cont) -> {
      return MovieDao_Impl.super.replaceAllForServer(serverId, entities, _cont);
    }, $completion);
  }
  ...
  public Object countForServer(final int serverId,
    ...
    final String _sql = "SELECT COUNT(*) FROM movies WHERE serverId = ?";
    return DBUtil.performSuspending(__db, true, false, (_connection) -> {
      final SQLiteStatement _stmt = _connection.prepare(_sql);
      try {
        int _argIndex = 1;
